...
jQuery( document ).on( "pagebeforecreate", "#page-my-yc-single", function( e ) {
  ...
  var build_list = function(){
    ...
    if( !is_empty(list_da) ){
      ...
      for( k in list_da ){
        ...
        var $li = jQuery('<li data-id="'+list_da[k].id+'" data-remote_id="'+(list_da[k].remote_id||'')+'"><a href="my_yc_'+page_key+'_view.html?id='+list_da[k].id+'&cat='+id+'" class="thumb"><img data-hash_id="'+list_da[k].id+'" data-img_pos="1" data-data_key="'+data_key+'" class="js_sync_img" src="images/thumb_default@2x.png"></a><a href="#delete_pop" data-rel="popup" data-position-to="window" data-transition="pop" class="btn_delete js_delete">Delete</a></li>');
  ...
  $page.on('pageshow',function(){
    ...
    //init pull
    if(!data_init[page_key+'-'+id]){
      pull_remote(function(){
        data_init[page_key+'-'+id] = true;
      });
    }else{
      build_list();
    }
...
/* 我的设置－》衣橱 －》 单品 ->浏览 */
jQuery( document ).on( "pagebeforecreate", "#page-my-yc-single-view", function( e ) {
  var $page = jQuery(e.target);
  ...
  $page.on("pagebeforeshow",function(e){
    ...
    var $btn_edit = $page.find('.js_edit');
    $btn_edit.attr('href','my_yc_single_add.html?id='+id+'&cat='+cat);
    ...
    if( id ){
      ...
      if( typeof list_da[id] == 'object' ){
        ...
        for( var k in cat_list ){
          if( cat_list[k].id == o['cat'] ){
            cat_da.push(cat_list[k].name);
          }
        }
        $cat.html(cat_da.join(',&nbsp;&nbsp;'));
        ...
        for( var k in brand_list ){
          if( brand_list[k].id == o['brand'] ){
            brand_da.push(brand_list[k].name);
          }
        }
        $brand.html(brand_da.join(',&nbsp;&nbsp;'));
        
        //颜色
        var color_list = get_o('my.settings.color');
        var color_da = [];
        for( var k in color_list ){
          if( color_list[k].id == o['color'] ){
            color_da.push(color_list[k].name);
          }
        }
        $color.text(color_da.join(',&nbsp;&nbsp;'));
        ...
        if( o['local_uri_1'] ){
          $photo_preview_1.data({
            'data_key':data_key,
            'hash_id':id,
            'img_pos':1
          }).addClass('js_sync_img');
        }
        
        if( o['local_uri_2'] ){
          $photo_preview_2.data({
            'data_key':data_key,
            'hash_id':id,
            'img_pos':2
          }).addClass('js_sync_img');
...
jQuery( document ).on( "pagebeforecreate", "#page-my-dp-list", function( e ) {
  ...
  var build_list = function(){
    ...
    if( !is_empty(list_da) ){
      ...
      for( k in list_da ){
        ...
        var a_href = "my_dp_view.html?id="+k;
        if(date){
          a_href += '&date='+date;
        }else if(scene){
          a_href += '&scene='+scene;
        ...
        */
        ...
        var $li = jQuery('<li data-id="'+list_da[k].id+'" data-remote_id="'+(list_da[k].remote_id||'')+'"><a href="'+a_href+'" class="thumb"><img data-hash_id="'+list_da[k].id+'" data-img_pos="first" data-data_key="'+data_key+'" class="js_sync_img" src="images/thumb_default@2x.png"></a><a href="#delete_pop" data-rel="popup" data-position-to="window" data-transition="pop" class="btn_delete js_delete">Delete</a></li>');
  ...
  $page.find('.fix_menu').each(function(index, element) {
    ...
    var $opts = $root.find('li[data-val]');
    ...
    $root.find('.li_lv1').each(function(index, element) {
      var $li_lv1 = jQuery(this);
      var $ul_lv2 = $li_lv1.find('.ul_lv2');
      
      if( $ul_lv2.length ){
        var $a_lv1 = $li_lv1.find('.span_lv1');
        $a_lv1.click(function(){
          if( $li_lv1.is('.open') ){
            $li_lv1.removeClass('open');
          }else{
            $li_lv1.addClass('open').siblings().removeClass('open');
          }
        });
      }
  ...
  $page.on('pagebeforeshow',function(e){
    ...
    if(date){
      ...
      $btn_add.attr('href','my_dp_add.html');
    }
  })
});

/* 我的设置－》衣橱 －》 搭配 ->浏览 */
jQuery( document ).on( "pagebeforecreate", "#page-my-dp-view", function( e ) {
  var $page = jQuery(e.target);
  var page_key = 'dapei';
  var data_key = 'my.dapei';
  ...
  $page.on("pageshow",function(e){
    ...
    $btn_edit.attr('href','my_dp_add.html?id='+id);
    ...
    $btn_back.attr('href','my_dp_list.html');
...
jQuery( document ).on( "pagebeforecreate", "#page-my-dp-add", function( e ) {
  ...
  $btn_save.click(function(){
    ...
    if( $li_choosen.length <2 ){
      ...
      jQuery.mobile.changePage('my_dp_list.html',{reloadPage:true,reverse:true});
  ...
  $page.on("pageshow",function(e){
    ...
    $preview_btn.each(function(index, element) {
      ...
      $this.attr('href','my_dp_single.html?id='+id+'&pos='+pos);
    });
  ...
  var create_list = function(){
    ...
    if( !is_empty(list_da) ){
      ...
      for( k in list_da ){
        ...
        var $li = jQuery('<li data-id="'+list_da[k].id+'" data-name="'+list_da[k].name+'" data-remote_id="'+(list_da[k].remote_id||'')+'"><a href="#" class="thumb"><img src="'+list_da[k].local_uri_1+'"></a></li>');
        
        var $li = jQuery('<li data-id="'+list_da[k].id+'" data-name="'+list_da[k].name+'" data-remote_id="'+(list_da[k].remote_id||'')+'"><a href="javascript:;" class="thumb"><img data-hash_id="'+list_da[k].id+'" data-img_pos="1" data-data_key="my.single" class="js_sync_img" src="images/thumb_default@2x.png"></a></li>');
        ...
        (function(){
          ...
          _$li.click(function(){
            ...
            if( date ){
              ...
              jQuery.mobile.changePage('my_dp_add.html?id='+id,{reloadPage:true,reverse:true});
            }
          })
        })();
        $list.append($li);
  ...
  */
  ...
  $page.on("pagebeforeshow",function(e){
    ...
    if(date){
      ...
      $btn_back.attr('href','my_dp_add.html?id='+id); 
    }
  });
      ...
      send_ajax( api_key+'.create',{
        ...
        if(data.status){
          upload_img(photo_uri,data.data,1,function(remote_uri){
            ...
            $content.val('');
            $photo_uri.val('');
            jQuery.mobile.changePage('sq_yc.html',{reloadPage:true,reverse:true});
          },function(){
            toast('图片上传失败，请稍候再试。');
          })
        }else{
...
jQuery( document ).on( "pagebeforecreate", "#page-sq-yc", function( e ) {
  ...
  var append_item = function( obj ){
    ...
    ' <div class="i">'+
    '   <span>喜欢：'+(obj.like||0)+'</span><span>评论：'+(obj.noc||0)+'</span><span>分享：'+(obj.rate||0)+'</span>'+
    ' </div>'+
    '</div>';
  ...
  var pull_remote = function(page_idx,_cb){
    ...
    var type = jQuery.trim($page.find('input[name=type]').val());
    if( type ){
      data.type = type;
    }
    
    
    send_ajax(api_key+'.list',data,function(data){
      if( data.status ){
...
jQuery( document ).on( "pagebeforecreate", "#page-sq-yc-detail", function( e ) {
  ...
  var update_content = function(_cb){
    ...
    send_ajax(api_key+'.detail',{'id':id},function(data){
      ...
      var type_name = '衣橱';
      var img_html = '  <div class="p"><a class="js_gallery_img" href="'+make_img_url(obj.id,1)+'"><img src="'+make_img_url(obj.id,1)+'" alt="" /></a></div>';
      switch(obj.type){
        case '1':
          type_name = '求搭';
          break;
      ...
      ' <div class="c">'+obj.description+'</div>'+
      ' <div class="i">'+
      '   <span class="js_like">喜欢：<em>'+(obj.like||0)+'</em></span><span class="js_comment">评论：<em>'+(obj.noc||0)+'</em></span><span class="js_share">分享：<em>'+(obj.rate||0)+'</em></span>'+
      ' </div>'+
      ...
      $li.find('.js_gallery_img').touchGallery();
      
      
      var $btn_comment = $li.find('.js_comment');
      $btn_comment.click(function(){
        var id = $page.data('id');
        jQuery.mobile.changePage( 'sq_yc_comment.html?id='+id,{reloadPage:true} );
      });
      
      var $btn_like = $li.find('.js_like');
      $btn_like.click(function(){
        
        if( $btn_like.is('.liked') ){
          toast('你已经喜欢过了。')
        }else{
          var id = $page.data('id');
          send_ajax('content.like',{'id':id},function(data){
            if(data.status){
              toast('喜欢成功！')
              $btn_like.find('em').text( parseInt($btn_like.find('em').text()) + 1 );
            }
          })
      ...
      var $btn_share = $li.find('.js_share');
      $btn_share.click(function(){
        var id = $page.data('id');
        
        window.plugins.socialsharing.share(obj.description,null,make_img_url(obj.id,1),null,function(){
          send_ajax('content.rate',{'id':id},function(){
            $btn_share.find('em').text( parseInt($btn_share.find('em').text()) + 1 );
          });
        });
      ...
      send_ajax( 'comment.list',{ target:obj.id },function(data){
        ...
        for( var i=0; i<data.data.content.length; i++ ){
          ...
          if( name ){
            var name_da = JSON.parse(name);
            if( name_da.tname ){
              name = obj.creator_name + ' 回复 ' +name_da.tname;
            }else{
          ...
          if(obj.files.length){
            ...
            '    </div>'+
            '   </div>'+
            '  <div class="ct"><a class="js_gallery_img" href="'+make_img_url(obj.id,1)+'"><img src="'+make_img_url(obj.id,1)+'" width="40" height="40" alt="" /></a></div>'+
            ' </div>'+
            ' <div class="cf">'+
            '  <span class="time">'+obj.time_create+'</span>'+
            '  <a href="javascript:;" class="btn_reply js_reply" data-name="'+obj.creator_name+'" data-id="'+obj.id+'">回复</a>'+
            ' </div>'+
            '</div>';
          }else{
            var ci_html = '<div class="ci ci_nt">'+
            ' <div class="cb">'+
            '  <div class="ccw">'+
            '   <div class="cc">'+
            '    <div class="tit">'+name+'</div>'+
          ...
          var $ci = jQuery(ci_html);
          
          $ci.appendTo($comment_list);
          
          if( $ci.find('.js_gallery_img').length ){
            $ci.find('.js_gallery_img').touchGallery();
          }
          
          var $btn_reply = $ci.find('.js_reply');
          $btn_reply.click(function(){
            var $this_btn = jQuery(this);
            var id = $page.data('id');
            var tid = $this_btn.data('id');
            var tname = $this_btn.data('name');
            jQuery.mobile.changePage( 'sq_yc_comment.html?id='+id+'&tid='+tid+'&tname='+tname,{reloadPage:true} );
          });
...
jQuery( document ).on( "pagebeforecreate", "#page-sq-yc-comment", function( e ) {
  ...
  var page_url = e.target.baseURI;
  var id = jQuery.url('?id',page_url) || '';  
  var tid = jQuery.url('?tid',page_url) || '';  
  var tname = jQuery.url('?tname',page_url) || '';  
  
  var name = '';
  if( tid && tname ){
    name = JSON.stringify({'tid':tid,'tname':tname});
  }
...
/* 关注 -> 用户搜索 */
jQuery( document ).on( "pagebeforecreate", "#page-user-find", function( e ) {
  var $page = jQuery(e.target);
  var $list = $page.find('.js_list');
  var $no_data = $page.find('.js_nodata');
  
  var $username = $page.find('.js_username');
  var $btn_search = $page.find('.js_btn_search');
	
  var $confirm_pop = $page.find('#confirm_pop');
  var $add = $page.find('.js_add');
	var $add_username = $page.find('.js_add_username')
	
  /* START DELETE action */
  $confirm_pop.find('.js_ok').click(function(){
    var $from = $confirm_pop.data('from_li');
    var user_id = $from.data('user_id');
    var username = $from.data('username');
    
    send_ajax('user.friend.create',{id:user_id},function(data){
			if( data.status ){
				toast('恭喜，您对'+username+'的朋友申请已发送成功！');
			}else{
				toast('抱歉，您已经对'+username+'发送过朋友申请。');
			}
  ...
  $btn_search.click(function(){
  	...
  	send_ajax('user.search',{name:username},function(data){
      $list.empty();
      ...
      if( data.data.content.length > 0 ){
        ...
        for( var i=0; i<data.data.content.length; i++ ){
          ...
          var $li = jQuery('<li data-icon="add" data-user_id="'+obj.id+'" data-username="'+obj.name+'"><a href="#confirm_pop" data-rel="popup" data-position-to="window" data-transition="pop">'+
          '<img src="images/thumb_default@2x.png">'+
          '<h2>'+obj.name+'</h2>'+
          '</a>'+
          '</li>');
					...
					$li.click(function(){
						...
						var name = $cur_li.data('username');
						$add_username.text(name);
						$confirm_pop.data('from_li',$cur_li);
					})
...
/* 关注 -> 朋友申请 */
jQuery( document ).on( "pagebeforecreate", "#page-user-friend-request", function( e ) {
  var $page = jQuery(e.target);
  var $list = $page.find('.js_list');
  var $no_data = $page.find('.js_nodata');
  ...
  /* START YES action */
  $confirm_pop.find('.js_yes').click(function(){
    var $from = $confirm_pop.data('from_li');
    var id = $from.data('id');
    var username = $from.data('username');
    
    send_ajax('user.friend.accept',{id:id},function(data){
			if( data.status ){
				toast('恭喜，您已通过'+username+'的朋友申请！');
			}else{
				//toast('抱歉，您已经对'+username+'发送过朋友申请。');
			}
  ...
  /* START NO action */
  $confirm_pop.find('.js_no').click(function(){
    var $from = $confirm_pop.data('from_li');
    var id = $from.data('id');
    var username = $from.data('username');
    
    send_ajax('user.friend.denied',{id:id},function(data){
			if( data.status ){
				toast('您已拒绝'+username+'的朋友申请。');
			}else{
				//toast('抱歉，您已经对'+username+'发送过朋友申请。');
			}
  ...
  /* END NO action */
	...
	$page.on('pagebeforeshow',function(){
  	...
  	send_ajax('user.friend.request',{},function(data){
      $list.empty();
      ...
      if( data.data.content.length > 0 ){
        ...
        for( var i=0; i<data.data.content.length; i++ ){
          ...
          var $li = jQuery('<li data-icon="add" data-id="'+obj.id+'" data-username="'+obj.creator_name+'"><a href="#confirm_pop" data-rel="popup" data-position-to="window" data-transition="pop">'+
          '<img src="images/thumb_default@2x.png">'+
          '<h2>'+obj.creator_name+'</h2>'+
          '</a>'+
          '</li>');
...
/* 关注 -> 朋友列表 */
jQuery( document ).on( "pagebeforecreate", "#page-user-friend", function( e ) {
  var $page = jQuery(e.target);
  var $list = $page.find('.js_list');
  var $no_data = $page.find('.js_nodata');
  ...
  $confirm_pop.find('.js_yes').click(function(){
    var $from = $confirm_pop.data('from_li');
    var r_id = $from.data('r_id');
    var username = $from.data('username');
    
    send_ajax('user.friend.denied',{id:r_id},function(data){
			if( data.status ){
				toast('您已删除好友'+username+'！');
				updat_list();
			}else{
				//toast('抱歉，您已经对'+username+'发送过朋友申请。');
			}
  ...
  /* END YES action */
	...
	var updat_list = function(){
		
		send_ajax('user.friend.list',{},function(data){
      $list.empty();
      ...
      if( data.data.content.length > 0 ){
        ...
        for( var i=0; i<data.data.content.length; i++ ){
          ...
          var $li = jQuery('<li data-icon="delete" data-r_id="'+obj.r_id+'" data-username="'+obj.name+'"><a href="#confirm_pop" data-rel="popup" data-position-to="window" data-transition="pop">'+
          '<img src="images/thumb_default@2x.png">'+
          '<h2>'+obj.name+'</h2>'+
          '</a>'+
          '</li>');
...
/* 护照 -> 脸型选择 */
jQuery( document ).on( "pagebeforecreate", "#page-ppc-lv0-face-s3", function( e ) {
  var $page = jQuery(e.target);
	
	var $btn_confirm = $page.find('.js_confirm');
	
	$btn_confirm.click(function(){
		jQuery.mobile.changePage( 'ppc_lv0_c1_face_s1.html',{reloadPage:true} );
	});
...
/* 护照 -> 体型选择 */
jQuery( document ).on( "pagebeforecreate", "#page-ppc-lv0-body-s2", function( e ) {
  var $page = jQuery(e.target);
	
	var $btn_confirm = $page.find('.js_confirm');
	
	$btn_confirm.click(function(){
		jQuery.mobile.changePage( 'ppc_lv0_c1_body_s1.html',{reloadPage:true} );
	});
